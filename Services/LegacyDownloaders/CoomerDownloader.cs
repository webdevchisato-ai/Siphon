...
namespace Siphon.Services.LegacyDownloaders
{
    public class CoomerDownloader
    {
        ...
        {
            _job.Status = "Initializing Coomer Browser...";
            ...
            // --- CHANGED LOGIC START ---
            // We now store tuples of (RelativeUrl, DisplayName)
            ...
            if (mainFile != null && mainFile["path"] != null)
            {
                ...
                string name = mainFile["name"]?.ToString(); // Get explicit name from API
            ...
            if (attachments != null)
            {
                foreach (var att in attachments)
                {
                    ...
                    string name = att["name"]?.ToString(); // Get explicit name from API
            ...
            foreach (var video in videosToDownload)
            {
                ...
                // Fallback: Try to get ?f=filename.mp4 from the URL path
                ...
                // Fallback: Use the hashed name from the path (e.g., a2579...adca.mp4)
        ...
        private bool IsVideo(string path)
        {
            ...
            // Check extension ignoring query strings
