        ...
        public async Task Download()
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                ...
                try
                {
                    ...
                    // Ensure browser is available (downloads if missing)
                    ...
                    browser = await Puppeteer.LaunchAsync(new LaunchOptions
                    {
                        ...
                        Args = new[] {
                            "--no-sandbox", 
                            // IMPORTANT: Proxy traffic via Tor
                    ...
                    try
                    {
                        await page.WaitForSelectorAsync("h1", new WaitForSelectorOptions { Timeout = 5000 });
                    ...
                    // Menu Trigger
                    var dlTrigger = await page.WaitForSelectorAsync("span[data-menutype='downloaddiv']", new WaitForSelectorOptions { Timeout = 10000 });
                    await dlTrigger.ClickAsync();
                    await page.WaitForSelectorAsync(".dloaddivcol", new WaitForSelectorOptions { Timeout = 5000 });
                    ...
                    var result = await Task.WhenAny(_signal.Task, Task.Delay(30000));
                    ...
                    // Handover to Shared Downloader
                    await SharedScraperLogic.DownloadWithProgressAsync(dlUrl, fullPath, "https://www.eporner.com/", name, attempt, _job);
                ...
                catch (Exception ex)
                {
                    ...
                    if (attempt == maxRetries) throw; // Rethrow on last fail
